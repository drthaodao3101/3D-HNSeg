...
class Trainer:
    ...
    def validate(self):
        ...
        if self.global_step % self.val_every == 0 \
            ...
            for i in tqdm(range(len(self.val_loader)), total=len(self.val_loader)):
                ...
                with torch.no_grad():
                    with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True) if (self.ddp or 'cuda' in self.device) else dummy_context():
    ...
    def save_checkpoint(self, checkpoint_path, **extra_state):
        """Save training checkpoint.
        
        Args:
            checkpoint_path: Path to save checkpoint
            **extra_state: Additional state to save (e.g., best_mean_dice)
        """
        if self.local_rank != 0:
        ...
        # Add any extra state from subclass (e.g., best_mean_dice)
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        Returns:
            Dictionary containing extra state that was saved
        """
        ...
        # Set epoch and global_step
        self.start_epoch = checkpoint.get('epoch', 0) + 1
        self.start_global_step = checkpoint.get('global_step', 0)
        
        # Return extra state for subclass to use
        extra_state = {k: v for k, v in checkpoint.items() 
                      if k not in ['epoch', 'global_step', 'model_state_dict', 
                                   'optimizer_state_dict', 'scheduler_state_dict', 
                                   'grad_scaler_state_dict']}
        
        print(f"Resuming from epoch {self.start_epoch}, global_step {self.start_global_step}")
        return extra_state
    ...
    def train(self,
        ...
        # Load checkpoint if provided (before creating scheduler)
        extra_state = {}
        if self.checkpoint_path:
            extra_state = self.load_checkpoint(self.checkpoint_path)
            # Subclass can override this to restore custom state
            self.on_load_checkpoint(extra_state)
        ...
        elif self.scheduler_type == "cosine_annealing_wr":
            from torch.optim.lr_scheduler import CosineAnnealingWarmRestarts
            lr = self.optimizer.state_dict()['param_groups'][0]['lr']
            print(f"initial lr is {lr}")
            self.scheduler = CosineAnnealingWarmRestarts(
                                                            self.optimizer,
                                                            T_0=int(self.max_epochs*0.4)*self.num_step_per_epoch,
                                                            T_mult=1,
                                                            eta_min=6e-6
                                                        )
            print(f"scheduler_type is Cosine Annealing Warm Restarts, T_0 is {int(self.max_epochs*0.4)*self.num_step_per_epoch} in steps")

        # Adjust global_step if resuming
        if self.checkpoint_path and hasattr(self, 'start_global_step'):
        ...
        if hasattr(self.train_loader, "stop"):
            self.train_loader.stop()

        if hasattr(self.val_loader, "stop"):
    ...
    def on_load_checkpoint(self, extra_state):
        """Override this method in subclass to restore custom state.
        
        Args:
            extra_state: Dictionary containing custom state from checkpoint
        """
        pass
