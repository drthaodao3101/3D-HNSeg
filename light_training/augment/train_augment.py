...
def get_train_transforms(patch_size, mirror_axes=None):
    ...
    border_mode_seg="constant", border_cval_seg=-1, order_seg=0,
    random_crop=False,  # random cropping is part of our dataloaders
    p_el_per_sample=0, p_scale_per_sample=0.4, p_rot_per_sample=0.4,
    independent_scale_for_each_axis=False  # todo experiment with this
    ))

    tr_transforms.append(GaussianNoiseTransform(p_per_sample=0.1))
    tr_transforms.append(GaussianBlurTransform((0.3, 0.7), different_sigma_per_channel=True, p_per_sample=0.15,
                                            p_per_channel=0.5))
    tr_transforms.append(BrightnessMultiplicativeTransform(multiplier_range=(0.8, 1.2), p_per_sample=0.15))
    tr_transforms.append(ContrastAugmentationTransform(contrast_range=(0.8, 1.2), p_per_sample=0.15))
    tr_transforms.append(SimulateLowResolutionTransform(zoom_range=(0.7, 1), per_channel=True,
                                                    p_per_channel=0.5,
                                                    order_downsample=0, order_upsample=3, p_per_sample=0.15,
                                                    ignore_axes=ignore_axes))
    tr_transforms.append(GammaTransform((0.8, 1.2), True, True, retain_stats=True, p_per_sample=0.15))
    tr_transforms.append(GammaTransform((0.8, 1.2), False, True, retain_stats=True, p_per_sample=0.15))
...
def get_train_transforms_backup(patch_size, mirror_axes=None):
    tr_transforms = []
    patch_size_spatial = patch_size
    ignore_axes = None
    angle = (-30. / 360 * 2. * np.pi, 30. / 360 * 2. * np.pi)
