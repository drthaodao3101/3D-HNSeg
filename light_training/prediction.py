...
class Predictor:
    ...
    def predict_raw_probability(model_output, properties):
        ...
        shape_after_cropping_before_resample = properties["shape_after_cropping_and_before_resampling"]
    ...
    def save_to_nrrd(self, return_output,
                 raw_spacing,
                 raw_origin,
                 raw_direction,
                 save_dir,
        ...
        if isinstance(raw_spacing[0], torch.Tensor):
            raw_spacing = [v.item() for v in raw_spacing]
        if isinstance(raw_origin[0], torch.Tensor):
            raw_origin = [v.item() for v in raw_origin]
        if isinstance(raw_direction[0], torch.Tensor):
            raw_direction = [v.item() for v in raw_direction]

        # Set metadata (SimpleITK expects x, y, z)
        return_output.SetSpacing(tuple(raw_spacing))
        return_output.SetOrigin(tuple(raw_origin))
        return_output.SetDirection(tuple(raw_direction))

        save_path = os.path.join(save_dir, f"{case_name}.nrrd")
