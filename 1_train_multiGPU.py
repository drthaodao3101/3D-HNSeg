...
class HeneCTTrainer(Trainer):
    def __init__(self, env_type, max_epochs, batch_size, roi_size,train_process=8, device="cpu", val_every=1, num_gpus=1,deep_supervision=False, logdir="./logs/",model_save_path="./logs/model/",augmentation=True, master_ip='localhost', master_port=17750, training_script="train.py"):
        super().__init__(env_type, max_epochs, batch_size, device, val_every, num_gpus, logdir, master_ip, master_port, training_script)
        self.window_infer = SlidingWindowInferer(roi_size=roi_size,
                                        sw_batch_size=1,
                                        overlap=0.5)
        
        from model_efficientmednext.mednextv1.EfficientMedNext_Full import EfficientMedNeXt_L
        from model_efficientmednext.mednextv1.EfficientMedNext import EfficientMedNeXt
        self.model = EfficientMedNeXt(
...
def parse_args():
    ...
    parser.add_argument("--not_call_launch", action="store_true")
